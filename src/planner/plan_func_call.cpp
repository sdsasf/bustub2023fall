...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  bustub::StringExpressionType expr_type;
  if (func_name == "lower") {
    expr_type = bustub::StringExpressionType::Lower;
  } else if (func_name == "upper") {
    expr_type = bustub::StringExpressionType::Upper;
  } else {
    throw Exception(fmt::format("func call {} not support in planner yet", func_name));
  }
  if (args.size() != 1) {
    throw Exception(fmt::format("func call {} only support 1 argument", func_name));
  }
  return std::make_shared<StringExpression>(args.front(), expr_type);
}
